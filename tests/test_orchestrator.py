...
async def test_incident_runs_end_to_end_and_opens_a_pull_request():
    ...
    # The PR body must tell a reviewer what to watch for after merge, and must
    # say who opened it. It must NOT be asserted to contain the word "review" —
    # that was the old check, and it passed happily while the footer above it
    # made a governance claim that turned out to be false.
    assert "Warden" in pr.body
    assert "no cluster credentials" in pr.body
