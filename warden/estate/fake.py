...
MODES = frozenset({"healthy", "bad_config", "oom", "blocked_rollout"})
...
class FakeAdapter:
    """Failure modes: `healthy`, `bad_config`, `oom`, `blocked_rollout`.

    `blocked_rollout` is the one a real cluster taught us to include: every
    replica ready and the workload still broken, because the ready replicas
    belong to the previous revision.
    """
    ...
    async def list_workloads(self, namespace: str) -> list[Workload]:
        ready = 3 if self.mode in {"healthy", "blocked_rollout"} else 0
        return [
            Workload(
                ref=CHECKOUT,
                replicas_desired=3,
    ...
    async def describe_workload(self, ref: WorkloadRef) -> WorkloadDetail:
        ...
        if self.mode == "blocked_rollout":
            # The shape that matters: one pod on the new (broken) revision,
            # three still serving on the old one.
            env["PAYMENT_ENDPOINT"] = "htps://payments.internal/v2"
            updated = 1
            containers = [
                ContainerState(name="checkout", ready=True, restart_count=0),
                ContainerState(
                    name="checkout",
                    ready=False,
                    restart_count=6,
            ...
            conditions = ["Available=True", "Progressing=False (ProgressDeadlineExceeded)"]
        elif self.mode == "bad_config":
    ...
    async def get_workload_logs(
        ...
        now = utcnow()
        if self.mode in {"bad_config", "blocked_rollout"}:
            lines = [
        ...
        return [
            LogLine(
                ts=now - timedelta(seconds=(len(lines) - i) * 5), container="checkout", message=m
            )
    ...
    async def query_metrics(
        ...
        return MetricSeries(
            ...
            points=[
                MetricPoint(ts=now - timedelta(minutes=m), value=base) for m in range(10, 0, -1)
            ],
        )

    async def get_workload_status(self, ref: WorkloadRef) -> Status:
        """Mirrors what the real adapter reports, including the awkward case.

        `blocked_rollout` is the shape the live cluster actually produced and
        the one that nearly sank a demo: every replica ready, and the workload
        in serious trouble, because the replicas that are ready belong to the
        OLD revision. It is here so that state is reachable offline, in tests,
        for free.
        """
        table = {
            "healthy": (True, "3/3 replicas ready", "complete", []),
            "bad_config": (
                False,
            ...
            "oom": (False, "0/3 replicas ready — OOMKilled", "blocked", ["OOMKilled"]),
            "blocked_rollout": (
                False,
        ...
        healthy, summary, rollout, reasons = table[self.mode]
        return Status(ref=ref, healthy=healthy, summary=summary, rollout=rollout, reasons=reasons)
